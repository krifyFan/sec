<template>
    <div>
        ...
        <Table border :columns="columns" :data="managements">
        ...
        <Modal v-model="flag_add" title="新增" @on-ok="addOk" @on-cancel="addCancel">
            <Form :model="addItem" :label-width="120">
                ...
                <FormItem label="等级">
                    <Input v-model="addItem.level" class="form-content-width"/>
                </FormItem>
                <FormItem label="父节点:">
                    <el-select v-model="addItem.fatherId" class="form-content-width">
                ...
                <FormItem label="地图:">
                    <el-select v-model="addItem.map" class="form-content-width">
        ...
        <Modal v-model="flag_edit" title="编辑" @on-ok="editOk" @on-cancel="editCancel">
            <Form :model="editItem" :label-width="120">
                ...
                <FormItem label="等级">
                    <Input v-model="editItem.level" class="form-content-width"/>
                </FormItem>
                <FormItem label="父节点:">
                    <el-select v-model="editItem.fatherId" class="form-content-width">
                        <el-option v-for="management in selectManagements" :value="management.id" :key="management.id"
                            :label="management.name"></el-option>
                    </el-select>
                </FormItem>
                ...
                <FormItem label="地图:">
                    <el-select v-model="editItem.map" class="form-content-width">
                        <el-option v-for="map in customs" :value="map.name" :key="map.id" :label="map.name"></el-option>
                    </el-select>
                </FormItem>
                <FormItem label="图标:">
...
<script lang="ts">
    ...
    export default class ManagementClass extends Vue {
        ...
        /** 下拉框中待选择的 */
        selectManagements: Management[] = [getDefaultManagement()]

        /** 待添加的管理部门对象 */
        addItem: Management = getDefaultManagement()

        /** 编辑窗口是否弹出 */
        flag_edit: boolean = false

        /** 待编辑的管理部门对象 */
        editItem: Management = getDefaultManagement()
        ...
        initData() {
            ...
            getPageInfo(pageParam).then((res: any) => {
                ...
                if (code == 200) {
                    ...
                    this.selectManagements = [getDefaultManagement()]
        ...
        addOk() {
            addManagement(this.addItem).then((res: any) => {
                ...
                if (code == 200) {
                    this.flag_add = false
                    this.addItem = getDefaultManagement()
                    this.initData()
                } else {
