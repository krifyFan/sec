<template>
    <div>
        ...
        <Table border :columns="columns" :data="alarmTypes">
            ...
            <Form :model="addItem" :label-width="120">
                ...
                <FormItem label="类别:">
                    <el-select v-model="addItem.category" class="form-content-width">
                        <el-option v-for="type in alarmCategory" :value="type.val" :key="type.val" :label="type.key">
                        </el-option>
                    </el-select>
                </FormItem>
                <FormItem label="级别:">
                    <el-select v-model="addItem.level" class="form-content-width">
                        <el-option v-for="type in alarmLevel" :value="type.val" :key="type.val" :label="type.key">
                        </el-option>
                    </el-select>
                </FormItem>
                <FormItem label="描述:">
                    <Input v-model="addItem.description" class="form-content-width"/>
                </FormItem>
            </Form>
        </Modal>
        <Modal v-model="flag_edit" title="编辑" @on-ok="editOk" @on-cancel="editCancel">
            <Form :model="editItem" :label-width="120">
                ...
                <FormItem label="级别:">
                    <el-select v-model="editItem.level" class="form-content-width">
                        <el-option v-for="type in alarmLevel" :value="type.val" :key="type.val" :label="type.key">
                        </el-option>
                    </el-select>
                </FormItem>
                <FormItem label="描述:">
                    <Input v-model="editItem.description" class="form-content-width"/>
                </FormItem>
            </Form>
        </Modal>
    </div>
...
<script lang="ts">
    ...
    export default class ManagementClass extends Vue {
        ...
        /** 告警等级 */
        alarmLevel: Array < EnumType > = this.$store.state.alarmLevel

        /** 告警类别 */
        alarmCategory: Array < EnumType > = this.$store.state.alarmCategory
            ...
            getPageInfo(pageParam).then((res: any) => {
                ...
                if (code == 200) {
                    ...
                    this.alarmTypes.map((item: AlarmType) => {
                        ...
                        let levelIndex: number = this.alarmLevel.findIndex(t => t.val == item.level)
                        item.levelName = levelIndex < 0 ? '' : this.alarmLevel[levelIndex].key

                        let categoryIndex: number = this.alarmCategory.findIndex(t => t.val == item.category)
                        item.categoryName = categoryIndex < 0 ? '' : this.alarmCategory[categoryIndex].key
                    })
                }
            })
        }
