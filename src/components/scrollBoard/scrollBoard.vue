<template>
    <div class="scroll-board" :ref="ref">
        <div class="header" v-if="header.length">
            <div class="header-item" v-for="(headerItem, i) in header" :key="`${headerItem}${i}`" :style="`height: ${mergedConfig.headerHeight}px;
        ...
        <div v-if="mergedConfig" class="rows"
            ...
            <div class="row-item" v-for="(row, ri) in rows" :key="`${row.toString()}${row.scroll}`" :style="`height: ${heights[ri]}px;
            ...
            `">
                <div class="ceil" v-for="(ceil, ci) in row.ceils" :key="`${ceil}${ri}${ci}`"
                    ...
                    @mouseleave="handleHover(false)"></div>
...
<script lang="ts">
    ...
    export default class Map extends Vue {
        ...
        propChange(newVal, oldVal) {
            if (oldVal.data.length !== 0) {
