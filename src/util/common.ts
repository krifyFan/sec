...
export default (function translateWH() {
    let whHash = {}
    let ratio = {}

    return {
        wh: function (decimal: number, integer: number) {

            if (!whHash[decimal + integer]) {
                whHash[decimal + integer] = Math.floor(decimal * integer / 100)
            }

            return whHash[decimal + integer]
        },
        ratio: function (ratioCoef: number, integer: number) {

            if (!ratio[ratioCoef + integer]) {
                ratio[ratioCoef + integer] = parseFloat(((ratioCoef / integer) * 100).toFixed(1))
            }

            return ratio[ratioCoef + integer]
        }
    }
})()

/**
 * @param {Object} root 深拷贝对象
 */
