    ...
    router.beforeEach(async (to: any, from: any, next: any) => {
        ...
        if (hasToken) {
            ...
            if (to.path === '/login' || to.path == '/') {
                ...
                if (store.state.managementId>0 && store.state.stationId== 0) {
                    next()
                } else {
                    next('/stations')
                    NProgress.done(); // 结束Progress
            ...
            if (code == 200) {
                ...
                roles.map((role: Role) => {
                    ...
                    role.alarmPermissionStr.split(',').forEach((s: string) => {
                        if (s)
                            role.permissions.push(Number(s))
                    })
                })
...
async function getEnum() {
    ...
    if (!store.state.alarmCategory) {
        let alarmCategory = await getEnumPromise(getAlarmCategory())
        store.commit('setAlarmCategory', alarmCategory)
    }

    if (!store.state.videoCategory) {
        let videoCategory = await getEnumPromise(getVideoCategory())
        store.commit("setVideoCategory", videoCategory)
    }

    if (!store.state.videoVendors) {
        let videoVendors = await getEnumPromise(getVideoVendors())
        store.commit("setVideoVendors", videoVendors)
    }
}
